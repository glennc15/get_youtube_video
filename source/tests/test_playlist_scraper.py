...
class GetVideosTest(unittest.TestCase):
	'''
	...
	def test_get_videos_test1(self):
		videos = get_videos(playlist_url=self.all_playlists_urls[0])
		self.assertTrue(len(videos) > 0)



	def test_get_videos_test2(self):
		videos = get_videos(playlist_url=self.all_playlists_urls[-1])
		self.assertTrue(len(videos) > 0)
...
# This class does not work in Colab so not using it for now:
